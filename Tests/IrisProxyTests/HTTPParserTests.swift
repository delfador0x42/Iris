...
struct ProxyCapturedFlowTests {
  ...
  func testFlowCompleteWithResponse() {
    ...
    let flow = ProxyCapturedFlow(
      flowType: .https, host: "example.com", port: 443,
  ...
  func testFlowCompleteWithError() {
    ...
    let flow = ProxyCapturedFlow(
      flowType: .https, host: "example.com", port: 443,
      request: request, error: "Connection refused"
    )
  ...
  func testFlowCodable() throws {
    ...
    let flow = ProxyCapturedFlow(
      flowType: .https, host: "api.example.com", port: 443,
      request: request, response: response,
      processName: "TestApp", processId: 12345
    )
