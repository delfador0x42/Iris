...
extension ESClient {
  ...
  func handleMmap(_ message: UnsafePointer<es_message_t>) {
    ...
    let info = extractBasicProcessInfo(from: proc)
    let flags = String(
      format: "prot=0x%x max=0x%x flags=0x%x", prot, event.max_protection, event.flags)
  ...
  func handleMprotect(_ message: UnsafePointer<es_message_t>) {
    ...
    let info = extractBasicProcessInfo(from: proc)
    let detail = String(
      format: "prot=0x%x addr=0x%llx size=0x%llx", prot, event.address, event.size)
  ...
  func handleAuthMprotect(_ client: OpaquePointer, _ message: UnsafePointer<es_message_t>) {
    ...
    if respondResult != ES_RESPOND_RESULT_SUCCESS {
      ...
      logger.error(
        "[AUTH] Failed to respond AUTH_MPROTECT for PID \(pid): \(respondResult.rawValue)")
    }
    ...
    if !decision.allow {
      ...
      logger.warning(
        "[AUTH] \(mode) MPROTECT W→X: \(procPath) pid=\(pid) reason=\(decision.reason)")
      let info = extractBasicProcessInfo(from: proc)
      let detail = String(
        format: "policy=%@ prot=0x%x addr=0x%llx", decision.reason, prot, event.address)
      recordSecurityEvent(.mprotect, process: info, detail: detail)
    }
  }
  ...
  func handleProcSuspendResume(_ message: UnsafePointer<es_message_t>) {
    ...
    recordSecurityEvent(
      .procSuspendResume, process: info, targetProcess: targetInfo, detail: typeStr)
  }
