...
extension ESClient {
  ...
  func handleAuthExec(_ client: OpaquePointer, _ message: UnsafePointer<es_message_t>) {
    ...
    let cache = decision.allow ? decision.cache : false  // Never cache denies

    let respondResult = es_respond_auth_result(client, message, result, cache)

    if respondResult != ES_RESPOND_RESULT_SUCCESS {
      logger.error(
        "[AUTH] Failed to respond to AUTH_EXEC for PID \(pid): \(respondResult.rawValue)")
    }
