...
//  IrisProxy
//
//  Main view for monitoring all network flows captured by the proxy.
//
...
public struct ProxyMonitorView: View {
  ...
  public var body: some View {
    ...
    .alert("Clear All Flows?", isPresented: $showingClearConfirmation) {
      ...
      Text(
        "This will remove all \(store.totalFlowCount) captured flows. This action cannot be undone."
      )
    }
  }

  @ViewBuilder
  func flowDetailView(for flow: ProxyCapturedFlow) -> some View {
    if flow.request != nil {
