...
extension DNSTabView {
            ...
            VStack(alignment: .leading, spacing: 20) {
                ...
                GroupBox {
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        HStack(spacing: 24) {
                            statMetric(
                                value: "\(store.totalQueries)",
                                label: "Total Queries",
                                color: .blue
                            )
                            statMetric(
                                value: String(format: "%.1fms", store.averageLatencyMs),
                                label: "Avg Latency",
                                color: store.averageLatencyMs < 50 ? .green : .orange
                            )
                            statMetric(
                                value: String(format: "%.1f%%", store.successRate * 100),
                                label: "Success Rate",
    ...
    private func statMetric(value: String, label: String, color: Color) -> some View {
        VStack(spacing: 4) {
            Text(value)
