...
- SysctlReader wraps sysctlbyname with type-safe generics
...
- IOKitRegistryReader.swift — IOKit device and driver enumeration
- SQLiteReader.swift — safe SQLite database reading (TCC.db, browser dbs)
- SysctlReader.swift — typed sysctl queries (kern.bootargs, hw.model, etc.)
- SocketEnumerator+AddressParsing.swift — IP address/port parsing for socket data
