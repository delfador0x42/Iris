...
public actor SecurityAssessor {
  ...
  public func assess() async -> (checks: [SecurityCheck], grade: SecurityGrade) {
    ...
    logger.info(
      "Assessment complete: \(grade.letter) (\(grade.score)/100), \(allChecks.count) checks")
  ...
  private func runThreatScanners() async -> [SecurityCheck] {
    ...
    anomalies.append(
      contentsOf: (await r14).map { finding in
        ProcessAnomaly(
    ...
    anomalies.append(
      contentsOf: (await r15).map { alert in
        ProcessAnomaly(
          ...
          description:
            "Encrypted \(alert.encryptedFiles.count) files (entropy: \(String(format: "%.2f", alert.entropy)))",
